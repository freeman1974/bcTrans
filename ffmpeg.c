...
static void log_callback_null(void *ptr, int level, const char *fmt, va_list vl)
{
	...
	if ((filesize>(800*1000)) ||(filesize==0))
		fp = fopen(logfile, "w+");
	else
		fp = fopen(logfile, "a+");

	if (fp) {
		sprintf(szTime, "TS%02d:%02d:%02d:",
...
int FF_main(int argc, char **argv)
{
	...
	//main thread for transcoding thread. never return if no error.
    if (transcode() < 0){
		ret=-4;goto End;
	}
...
End:
	av_log(NULL, AV_LOG_INFO, "exit %s(),ret=%d\n",__FUNCTION__,ret);
    exit_program(ret);
    return ret;
}
